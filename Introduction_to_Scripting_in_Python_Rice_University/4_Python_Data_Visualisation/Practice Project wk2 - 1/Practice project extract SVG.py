...
def get_county_attributes(svg_file_name):
    ...
    svg_doc = minidom.parse(svg_file_name) #parse file
    county_attribute_list = [] #create empty list
    for path in svg_doc.getElementsByTagName('path'): #for every path in doc
    ...
    svg_doc.unlink() # always unlick connection
...
def test_get_attributes(svg_file_name):
    ...
    print("file length: ", len(county_attribute_list))
    print()
